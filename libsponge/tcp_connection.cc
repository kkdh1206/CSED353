...
void TCPConnection::segment_received(const TCPSegment &seg) {  // 수신시
    ...
    if (!_connected) {  // 무시만하면됨 그뒤에 정상적인거 기다리고
        if (!seg.header().syn) {
            return;  // SYN이 아니면 무시 시작이 연결도 안됬으니까
        }
    }
    ...
    if (_receiver.stream_out().input_ended() &&
        !_sender.stream_in().eof()) {  // FIN을 받은경우는 꺼줌, 내가 FIN을 보낸경우에는 계속 true로 둠
        _linger_after_streams_finish = false;
    }

    if (seg.header().syn &&
        !_connected) {  //_sender.next_seqno_absolute()== 0) { // 처음 SYN에 대해서만 connect날리고 SYNACK는 패스시킴
        connect();      // 연결요청
        return;
    }
    ...
    if (seg.length_in_sequence_space() > 0 ||
        ((_receiver.ackno().has_value() && (seg.length_in_sequence_space() == 0) &&
          seg.header().seqno ==
              _receiver.ackno().value() -
                  1))) {  //  keep-alive - 이전번호를 보내줘서 연결살아있는지 확인용이고 크기가 0이라서 이 조건추가해줌
...
void TCPConnection::tick(const size_t ms_since_last_tick) {
    ...
    if (_sender.consecutive_retransmissions() <= TCPConfig::MAX_RETX_ATTEMPTS) {
        if (_sender.next_seqno_absolute() >
            0) {  // 그냥 tick할때 계속 fill_window돌필요는없음 안올때도 뭘 보낼필요는없으니까 !!!
            // 이게 핵심이었나
            _sender.fill_window();  // zero probing 용
        }
        ...
        if (_sender.next_seqno_absolute() > 0 || _receiver.ackno().has_value()) {
            ...
            seg.header().win = win;
            if (ackno.has_value()) {
                seg.header().ackno = ackno.value();  // syn 일때는 ackno없음 그냥 isn값만 seqno에 실어서 보냄, 이제 이걸
                                                     // 받은쪽이 isn설정됬으니가 ackno응답이 가능해서 실어서줌
...
void TCPConnection::connect() {  // 연결하는 함수 3hand shake 의 syn보내는 첫부분만 담당
    ...
    _connected = true;
    if (_sender.next_seqno_absolute() >
        0) {  // 방어로직 - 이미 syn을 보냈으면 1이상이라서 더이상 connect호출해도 무시해야함
        return;
    }
    _sender.fill_window();
    send_segment();
...
void TCPConnection::send_segment() {  // queue에 쌓인걸 보내는함수
    auto &queue =
        _sender.segments_out();  // auto로 하면 타입 신경안써도 됨 그리고 & 붙여서 참조로 가져와야 pop이 먹힘 복사안해서
    ...
    try {
        if (active()) {
            ...
            if (_sender.next_seqno_absolute() > 0 || _receiver.ackno().has_value()) {
                ...
                seg.header().win = win;
                if (ackno.has_value()) {
                    seg.header().ackno = ackno.value();  // syn 일때는 ackno없음 그냥 isn값만 seqno에 실어서 보냄, 이제
                                                         // 이걸 받은쪽이 isn설정됬으니가 ackno응답이 가능해서 실어서줌
                    // optional 타입이라 값을 받으려면 .value()로 까야함
                    seg.header().ack = true;
                }
