...
void StreamReassembler::push_substring(const string &data,
                                       const size_t index,
                                       const bool eof) {  // 데이터 들어오는거 처리
    size_t lower = this->last_index;                      // 여기서부터 채워나가야함
    size_t upper = _output.bytes_read() +
                   _capacity;  // 이걸로 존재할수있는 영역 바운드 - 이때 오해하면안되는게 lower~upper가 capcity
                               // 만큼있는게 아님 그냥정렬되서 읽는거로 넘긴것뿐임 넘겨도 같은 capcity공유
    string local_data = data;
    size_t local_index = index;
    // 내 앞 map 만 겹치는지 보면됨 왜냐면 서로 안겹치니까 그리고 내 바로 뒷 map과 겹치면 거기까지만 저장

    if (eof && index + data.length() <=
                   upper) {  // eof가 먼저 도착할수도있는데 그때 먼저 꺼버리면 안됨 - 대신ㄴ 제대로 들어온거만 기억함
    ...
    if (_is_eof_set &&
        last_index == _eof_idx) {  // 실제 last_index가 최종 index인 eof_index에 도달하고 eof가 온경우면 종료
        _output.end_input();
    }
}
