...
uint64_t TCPSender::bytes_in_flight() const { 
    return _bytes_in_flight; } // 이걸 관리해서 더해주긴해야함
...
void TCPSender::fill_window() {
    ...
    while (win > _bytes_in_flight) {
        ...
        // 남은공간, payload 최대중에 작은거 고름
        size_t read_size = min({static_cast<size_t>(space), 
                                _stream.buffer_size(), 
                                TCPConfig::MAX_PAYLOAD_SIZE});
        // 이걸통해서 그냥 헤더에 신호만 보내는 케이스가 아닐때 데이터 보낼때만 데이터 싣어줌
        if (read_size > 0) seg.payload() = Buffer(_stream.read(read_size));
...
void TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) { 
    ...
    bool check = false;
    while (!_outstanding_segments.empty() && unwrap(_outstanding_segments.front().header().seqno + _outstanding_segments.front().length_in_sequence_space(),_isn,_next_seqno) <= abs_seqno){
...
void TCPSender::tick(const size_t ms_since_last_tick) { 
    ...
    if (_retransmission_timeout <= _time){ // 이러면 타임아웃임 - 등호포함해야함
        ...
        if (_window_size > 0) { _retransmission_timeout *= 2; }
        _time =0; // 시간초기화
    }
...
void TCPSender::send_empty_segment() { // ackno만 보내고 싶을때?
    TCPSegment empty;
    empty.header().seqno = next_seqno(); // 보낼때 absoulte seqno로 보내는게아니라 seqno로 보낸다 이는 32비트로 더 경제적으로 보낼수있고 이렇게보내도 처음에 보내는쪽의 isn을 미리저장해두기 때문에 복구가 가능해서 가능하다.
